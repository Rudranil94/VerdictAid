...
RUN pip install --no-cache-dir --upgrade pip && \
    ...
    pip install --no-cache-dir --no-deps "fastapi==0.109.1" "uvicorn==0.27.0" "python-multipart==0.0.20" \
    ...
    pip install --no-cache-dir --no-deps "fastapi-users[sqlalchemy]==14.0.1" "bcrypt==4.1.2" && \
    # Install document processing dependencies
    pip install --no-cache-dir --no-deps "langchain-community==0.1.0" "langchain==0.1.0" "openai==1.10.0" \
    "pypdf2==3.0.1" "python-docx==1.0.1" "spacy==3.7.4" && \
    # Install notifications dependencies
    pip install --no-cache-dir --no-deps "aiosmtplib==3.0.1" "jinja2==3.1.3" "pywebpush==1.14.0" "firebase-admin==6.3.0" && \
    # Install testing dependencies
    pip install --no-cache-dir --no-deps "pytest==7.4.3" "pytest-asyncio==0.23.4" "pytest-cov==4.1.0" "pytest-mock==3.12.0" \
    ...
    pip install --no-cache-dir --no-deps "safety>=2.3.5" "bandit==1.7.6" && \
    # Install development dependencies
    pip install --no-cache-dir --no-deps "black==24.1.1" "isort==5.13.2"
