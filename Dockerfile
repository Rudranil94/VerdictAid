...
RUN pip install --no-cache-dir --upgrade pip && \
    ...
    pip install --no-cache-dir "python-jose[cryptography]==3.3.0" "passlib[bcrypt]==1.7.4" "pydantic==2.5.3" "pydantic-settings==2.1.0" && \
    # Install database dependencies
    pip install --no-cache-dir "sqlalchemy==2.0.25" "alembic==1.13.1" "asyncpg==0.29.0" && \
    # Install Redis and Celery
    pip install --no-cache-dir "redis==5.0.1" "celery==5.3.6" && \
    ...
    pip install --no-cache-dir "pytest-xdist==3.5.0" "httpx==0.26.0" "aioresponses==0.7.6" "freezegun==1.4.0" "faker==22.6.0" "locust==2.20.1" && \
